...
restData <- function(als_data = als_data, speed_threshold = 15, pct_movement = 0.05, day = "7:00", night = "19:00") {
  ...
  write.csv(rest_data$bout_data, paste(als_data$sample_id, "_bout_structure.csv"))
  write.csv(rest_data$daily_summary, paste(als_data$sample_id, "_daily_bout_summary.csv"))
  write.csv(rest_data$weekly_summary, paste(als_data$sample_id, "_weekly_bout_summary.csv"))
