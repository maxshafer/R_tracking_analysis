...
saveRDS(meta_data, file = here("meta_data_all.rds"))


# saveRDS(als.files, file = here("als_file_list.rds"))
...
als.files <- list.files(path = here(), recursive = TRUE, pattern = "als.csv")
names <- c(names(als.data.list), als.files)

als.data.list[(length(als.data.list)+1):(length(als.data.list) + length(als.files))] <- lapply(als.files, function(x) loadALSfiles(path_to_file = x, average_by = "minute"))

names(als.data.list) <- names

saveRDS(als.data.list, file = here("all_fish_als_combined.rds"))
